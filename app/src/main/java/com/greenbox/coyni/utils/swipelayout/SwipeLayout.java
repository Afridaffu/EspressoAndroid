...
public class SwipeLayout extends FrameLayout {
    ...
    private static final SwipeLayout.DragEdge DefaultDragEdge = SwipeLayout.DragEdge.Right;

    private int mTouchSlop;

    private SwipeLayout.DragEdge mCurrentDragEdge = DefaultDragEdge;
    private ViewDragHelper mDragHelper;

    private int mDragDistance = 0;
    private LinkedHashMap<SwipeLayout.DragEdge, View> mDragEdges = new LinkedHashMap<SwipeLayout.DragEdge, View>();
    private SwipeLayout.ShowMode mShowMode;

    private float[] mEdgeSwipesOffset = new float[4];

    private List<SwipeLayout.SwipeListener> mSwipeListeners = new ArrayList<SwipeLayout.SwipeListener>();
    private List<SwipeLayout.SwipeDenier> mSwipeDeniers = new ArrayList<SwipeLayout.SwipeDenier>();
    private Map<View, ArrayList<SwipeLayout.OnRevealListener>> mRevealListeners = new HashMap<View, ArrayList<SwipeLayout.OnRevealListener>>();
    ...
    public SwipeLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mEdgeSwipesOffset[SwipeLayout.DragEdge.Left.ordinal()] = a.getDimension(R.styleable.SwipeLayout_leftEdgeSwipeOffset, 0);
        mEdgeSwipesOffset[SwipeLayout.DragEdge.Right.ordinal()] = a.getDimension(R.styleable.SwipeLayout_rightEdgeSwipeOffset, 0);
        mEdgeSwipesOffset[SwipeLayout.DragEdge.Top.ordinal()] = a.getDimension(R.styleable.SwipeLayout_topEdgeSwipeOffset, 0);
        mEdgeSwipesOffset[SwipeLayout.DragEdge.Bottom.ordinal()] = a.getDimension(R.styleable.SwipeLayout_bottomEdgeSwipeOffset, 0);
        ...
        int ordinal = a.getInt(R.styleable.SwipeLayout_show_mode, SwipeLayout.ShowMode.PullOut.ordinal());
        mShowMode = SwipeLayout.ShowMode.values()[ordinal];
        a.recycle();
    ...
    public void addRevealListener(int childId, SwipeLayout.OnRevealListener l) {
        ...
        if (mRevealListeners.get(child) == null)
            mRevealListeners.put(child, new ArrayList<SwipeLayout.OnRevealListener>());
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
            ...
            if (child == getSurfaceView()) {
                ...
                switch (mCurrentDragEdge) {
                    ...
                    case Bottom:
                        return getPaddingLeft();
                    case Left:
                        if (mShowMode == SwipeLayout.ShowMode.PullOut) {
                            if (left > getPaddingLeft()) return getPaddingLeft();
                        }
                        break;
                    case Right:
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if(result){
                isCloseBeforeDrag = getOpenStatus() == SwipeLayout.Status.Close;
            }
            return result;
        }
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            for (SwipeLayout.SwipeListener l : mSwipeListeners){
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if (changedView == surfaceView) {
                ...
                if (mShowMode == SwipeLayout.ShowMode.PullOut && currentBottomView!=null) {
                    if (mCurrentDragEdge == SwipeLayout.DragEdge.Left || mCurrentDragEdge == SwipeLayout.DragEdge.Right){
                ...
                if (mShowMode == SwipeLayout.ShowMode.PullOut) {
                    ...
                    if (mCurrentDragEdge == SwipeLayout.DragEdge.Left && newLeft < getPaddingLeft())
                        newLeft = getPaddingLeft();
                    else if (mCurrentDragEdge == SwipeLayout.DragEdge.Right && newLeft > getPaddingLeft())
                        newLeft = getPaddingLeft();
                    else if (mCurrentDragEdge == SwipeLayout.DragEdge.Top && newTop < getPaddingTop())
                        newTop = getPaddingTop();
                    else if (mCurrentDragEdge == SwipeLayout.DragEdge.Bottom && newTop > getPaddingTop())
                        newTop = getPaddingTop();
     ...
     */
    protected boolean isViewTotallyFirstShowed(View child, Rect relativePosition, SwipeLayout.DragEdge edge, int surfaceLeft,
        ...
        if (getShowMode() == SwipeLayout.ShowMode.LayDown) {
            if ((edge == SwipeLayout.DragEdge.Right && surfaceRight <= childLeft)
                    || (edge == SwipeLayout.DragEdge.Left && surfaceLeft >= childRight)
                    || (edge == SwipeLayout.DragEdge.Top && surfaceTop >= childBottom)
                    || (edge == SwipeLayout.DragEdge.Bottom && surfaceBottom <= childTop)) r = true;
        } else if (getShowMode() == SwipeLayout.ShowMode.PullOut) {
            if ((edge == SwipeLayout.DragEdge.Right && childRight <= getWidth())
                    || (edge == SwipeLayout.DragEdge.Left && childLeft >= getPaddingLeft())
                    || (edge == SwipeLayout.DragEdge.Top && childTop >= getPaddingTop())
                    || (edge == SwipeLayout.DragEdge.Bottom && childBottom <= getHeight())) r = true;
        }
        return r;
    }

    protected boolean isViewShowing(View child, Rect relativePosition, SwipeLayout.DragEdge availableEdge, int surfaceLeft,
                                    int surfaceTop, int surfaceRight, int surfaceBottom) {
        int childLeft = relativePosition.left;
        int childRight = relativePosition.right;
        int childTop = relativePosition.top;
    ...
    protected void dispatchSwipeEvent(int surfaceLeft, int surfaceTop, int dx, int dy) {
        SwipeLayout.DragEdge edge = getDragEdge();
        boolean open = true;
        if (edge == SwipeLayout.DragEdge.Left) {
            ...
            if (dy < 0) open = false;
        } else if (edge == SwipeLayout.DragEdge.Bottom) {
            if (dy > 0) open = false;
        }
    ...
    protected void dispatchSwipeEvent(int surfaceLeft, int surfaceTop, boolean open) {
        safeBottomView();
        SwipeLayout.Status status = getOpenStatus();
    ...
    protected void dispatchRevealEvent(final int surfaceLeft, final int surfaceTop, final int surfaceRight,
        ...
        for (Map.Entry<View, ArrayList<SwipeLayout.OnRevealListener>> entry : mRevealListeners.entrySet()) {
            ...
            if (isViewShowing(child, rect, mCurrentDragEdge, surfaceLeft, surfaceTop,
                ...
                for (SwipeLayout.OnRevealListener l : entry.getValue()) {
            ...
            if (isViewTotallyFirstShowed(child, rect, mCurrentDragEdge, surfaceLeft, surfaceTop,
                ...
                mShowEntirely.put(child, true);
                for (SwipeLayout.OnRevealListener l : entry.getValue()) {
                    if (mCurrentDragEdge == SwipeLayout.DragEdge.Left
                            || mCurrentDragEdge == SwipeLayout.DragEdge.Right)
    ...
    private List<SwipeLayout.OnLayout> mOnLayoutListeners;

    public void addOnLayoutListener(SwipeLayout.OnLayout l) {
        if (mOnLayoutListeners == null) mOnLayoutListeners = new ArrayList<SwipeLayout.OnLayout>();
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        if(gravity>0){
            ...
            if((gravity & Gravity.LEFT) == Gravity.LEFT){
                mDragEdges.put(SwipeLayout.DragEdge.Left, child);
            }
            if((gravity & Gravity.RIGHT) == Gravity.RIGHT){
                mDragEdges.put(SwipeLayout.DragEdge.Right, child);
            }
            if((gravity & Gravity.TOP) == Gravity.TOP){
                mDragEdges.put(SwipeLayout.DragEdge.Top, child);
            }
            if((gravity & Gravity.BOTTOM) == Gravity.BOTTOM){
                mDragEdges.put(SwipeLayout.DragEdge.Bottom, child);
            }
        }else{
            for(Map.Entry<SwipeLayout.DragEdge, View> entry : mDragEdges.entrySet()){
    ...
    void layoutPullOut() {
        ...
        rect = computeBottomLayoutAreaViaSurface(SwipeLayout.ShowMode.PullOut, rect);
        View currentBottomView = getCurrentBottomView();
        if(currentBottomView!=null){
            currentBottomView.layout(rect.left, rect.top, rect.right, rect.bottom);
        }
    ...
    void layoutLayDown() {
        ...
        rect = computeBottomLayoutAreaViaSurface(SwipeLayout.ShowMode.LayDown, rect);
        View currentBottomView = getCurrentBottomView();
        if(currentBottomView!=null){
            currentBottomView.layout(rect.left, rect.top, rect.right, rect.bottom);
        }
    ...
    private void checkCanDrag(MotionEvent ev){
        ...
        if (mCurrentDragEdge == SwipeLayout.DragEdge.Right) {
            boolean suitable = (status == SwipeLayout.Status.Open && distanceX > mTouchSlop)
                    || (status == SwipeLayout.Status.Close && distanceX < -mTouchSlop);
        ...
        if (mCurrentDragEdge == SwipeLayout.DragEdge.Top) {
            boolean suitable = (status == SwipeLayout.Status.Open && distanceY < -mTouchSlop)
                    || (status == SwipeLayout.Status.Close && distanceY > mTouchSlop);
            suitable = suitable || status == SwipeLayout.Status.Middle;
        ...
        if (mCurrentDragEdge == SwipeLayout.DragEdge.Bottom) {
            boolean suitable = (status == SwipeLayout.Status.Open && distanceY > mTouchSlop)
                    || (status == SwipeLayout.Status.Close && distanceY < -mTouchSlop);
            suitable = suitable || status == SwipeLayout.Status.Middle;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if(mClickToClose && getOpenStatus() == SwipeLayout.Status.Open && isTouchOnSurface(ev)){
            return true;
        }
        for (SwipeLayout.SwipeDenier denier : mSwipeDeniers) {
    ...
    public boolean isLeftSwipeEnabled() {
        View bottomView = mDragEdges.get(SwipeLayout.DragEdge.Left);
        return bottomView != null && bottomView.getParent() == this
                && bottomView != getSurfaceView() && mSwipesEnabled[SwipeLayout.DragEdge.Left.ordinal()];
    }
    ...
    public boolean isRightSwipeEnabled() {
        View bottomView = mDragEdges.get(SwipeLayout.DragEdge.Right);
        return bottomView!=null && bottomView.getParent()==this
                && bottomView != getSurfaceView() && mSwipesEnabled[SwipeLayout.DragEdge.Right.ordinal()];
    }
    ...
    public boolean isTopSwipeEnabled() {
        View bottomView = mDragEdges.get(SwipeLayout.DragEdge.Top);
        return bottomView!=null && bottomView.getParent()==this
                && bottomView != getSurfaceView() && mSwipesEnabled[SwipeLayout.DragEdge.Top.ordinal()];
    }
    ...
    public boolean isBottomSwipeEnabled() {
        View bottomView = mDragEdges.get(SwipeLayout.DragEdge.Bottom);
        return bottomView!=null && bottomView.getParent()==this
                && bottomView != getSurfaceView() && mSwipesEnabled[SwipeLayout.DragEdge.Bottom.ordinal()];
    }
    ...
    private void performAdapterViewItemClick() {
        if(getOpenStatus()!= SwipeLayout.Status.Close) return;
        ViewParent t = getParent();
        if (t instanceof AdapterView) {
    ...
    private boolean performAdapterViewItemLongClick() {
        if(getOpenStatus()!= SwipeLayout.Status.Close) return false;
        ViewParent t = getParent();
        if (t instanceof AdapterView) {
            AdapterView view = (AdapterView) t;
            int p = view.getPositionForView(SwipeLayout.this);
    ...
    private GestureDetector gestureDetector = new GestureDetector(getContext(), new SwipeLayout.SwipeDetector());
    ...
    public List<View> getBottomViews() {
        ...
        for(SwipeLayout.DragEdge dragEdge : SwipeLayout.DragEdge.values()){
    ...
    public SwipeLayout.Status getOpenStatus() {
        ...
        if (surfaceLeft == getPaddingLeft() && surfaceTop == getPaddingTop()) return SwipeLayout.Status.Close;
    ...
    protected void processHandRelease(float xvel, float yvel, boolean isCloseBeforeDragged) {
        ...
        View surfaceView = getSurfaceView();
        SwipeLayout.DragEdge currentDragEdge = mCurrentDragEdge;
    ...
    public void open(boolean smooth, boolean notify) {
        ...
        if (smooth) {
            ...
            if (getShowMode() == SwipeLayout.ShowMode.PullOut) {
                Rect bRect = computeBottomLayoutAreaViaSurface(SwipeLayout.ShowMode.PullOut, rect);
                if(bottom!=null){
    ...
    public void toggle(boolean smooth) {
        if (getOpenStatus() == SwipeLayout.Status.Open)
            close(smooth);
    ...
    private Rect computeSurfaceLayoutArea(boolean open) {
        ...
        if (open) {
            ...
            else if (mCurrentDragEdge == SwipeLayout.DragEdge.Right)
                l = getPaddingLeft() - mDragDistance;
            else if (mCurrentDragEdge == SwipeLayout.DragEdge.Top)
    ...
    /**Deprecated, use {@link #addDrag(SwipeLayout.DragEdge, View)} */
    @Deprecated
    ...
    public void onViewRemoved(View child) {
        for(Map.Entry<SwipeLayout.DragEdge, View> entry : new HashMap<SwipeLayout.DragEdge, View>(mDragEdges).entrySet()){
    ...
    public List<SwipeLayout.DragEdge> getDragEdges() {
        return new ArrayList<SwipeLayout.DragEdge>(mDragEdges.keySet());
    }

    /**Deprecated, use {@link #addDrag(SwipeLayout.DragEdge, View)} */
    @Deprecated
    public void setDragEdges(List<SwipeLayout.DragEdge> dragEdges) {
        for (int i = 0, size = Math.min(dragEdges.size(), getChildCount() - 1); i < size; i++) {
            SwipeLayout.DragEdge dragEdge = dragEdges.get(i);
    ...
    public void setBottomViewIds(int leftId, int rightId, int topId, int bottomId) {
        addDrag(SwipeLayout.DragEdge.Left, findViewById(leftId));
        addDrag(SwipeLayout.DragEdge.Right, findViewById(rightId));
        addDrag(SwipeLayout.DragEdge.Top, findViewById(topId));
        addDrag(SwipeLayout.DragEdge.Bottom, findViewById(bottomId));
    }
    ...
    private void updateBottomViews() {
        ...
        if (mShowMode == SwipeLayout.ShowMode.PullOut)
            layoutPullOut();
        else if (mShowMode == SwipeLayout.ShowMode.LayDown) layoutLayDown();
